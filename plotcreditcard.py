...
def load_and_plot_credit_card_results():
    ...
    files = [
        'creditcard_data_r10_results.csv',
    ]
    ...
    print(f"Found data for k values: {r_values}")
    ...
    for r in r_values:
        ...
        plt.title(f'Credit Card Data - Cumulative Loss (r={10})', fontsize=18)
        plt.legend(fontsize=14)
        plt.tick_params(axis='both', which='major', labelsize=14)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        filename = f'creditcard_r{r}_cumulative_loss.png'
