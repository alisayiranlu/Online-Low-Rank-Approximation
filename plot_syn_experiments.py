...
def plot_synthetic_cumulative_comparison():
    ...
    r_expert_values = [2, 5, 10]  # Changed from k_values to r_expert_values
   
    data = {}
    for dataset in datasets:
        data[dataset] = {}
        for r_expert in r_expert_values:  # Changed from k to r_expert
            filename = f'{dataset}_data_r{r_expert}_results.csv'  # Changed filename format
            try:
                df = pd.read_csv(filename)
                data[dataset][r_expert] = df
    ...
    for dataset in datasets:
        for r_expert in r_expert_values:  # Changed from k to r_expert
            if r_expert not in data[dataset]:
                continue
               
            df = data[dataset][r_expert]
            ...
            plt.title(f'{dataset.title()} Data - Algorithm Performance (r={r_expert})', fontsize=22)  # Simplified title with r instead of r_expert
            plt.legend(fontsize=18)
            plt.tick_params(axis='both', which='major', labelsize=18)
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
           
            filename = f'synthetic_{dataset}_r{r_expert}_cumulative_loss.png'  # Changed filename format
    ...
    print("Fixed k=5, varying r_expert values")  # Added clarification
    print("="*60)
    ...
    for dataset in datasets:
        ...
        for r_expert in r_expert_values:  # Changed from k to r_expert
            if r_expert in data[dataset]:
                ...
                print(f"  r_expert={r_expert}:")  # Changed from k to r_expert
